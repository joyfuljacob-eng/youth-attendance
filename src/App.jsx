          ...
          {selectedSam&&(
            <>
              <div className="sam-card">
                ...
                <div className="sam-info"><div className="sam-name">{sams.find(s=>s.id===selectedSam)?.name}샘</div><div className="sam-count">구성원 {samMembers.length}명</div></div>
              ...
              {tab==="check"?(
                <>
                  <div className="date-row">
                    <input type="date" className="date-input-styled" value={selectedDate} onChange={e=>setSelectedDate(e.target.value)}/>
                    <div style={{textAlign:"right",flexShrink:0}}><div style={{fontSize:18,fontWeight:700,color:"#10B981"}}>{presentCount}</div><div style={{fontSize:11,color:"#94A3B8"}}>/ {samMembers.length}명</div></div>
                  </div>
                  {samMembers.length===0?(<div className="empty-state"><div className="empty-state-icon">👤</div><div className="empty-state-text">배정된 청년이 없습니다</div><div className="empty-state-sub">청년 명단에서 샘을 배정해주세요</div></div>):(
                    samMembers.map(m=>{
                      const present=isPresent(m.id);
                      return(
                  ...
                  {allDates.length===0?(<div className="empty-state"><div className="empty-state-icon">📊</div><div className="empty-state-text">출석 기록이 없습니다</div></div>):(
                    <div style={{overflowX:"auto"}}>
                      <table className="summary-table">
                        ...
                        <tbody>{samMembers.map(m=><tr key={m.id}><td>{m.name}</td>{allDates.map(d=>{const rec=samAttendanceList.find(a=>a.member_id===m.id&&a.sam_id===selectedSam&&a.date===d);return<td key={d}>{rec?.status?<span className="dot-present">✓</span>:<span className="dot-absent"/>}</td>;})}</tr>)}</tbody>
                      </table>
                    </div>
                  )}
                </>
